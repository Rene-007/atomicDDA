function fftA = create_fftA_gpu(k,r)
...
%  The individual elements are calculated after Schmehl 1997 -- Appendix 3
    ...
    r_center = gpuArray(repmat(r(center,:),N,1));             % center position replicated as a matrix
    r_all = gpuArray(r([center:N, 1:center-1],:));            % all the other vectors in a circulant arrangement
...
%     Aj(1:N,9) = C.*(gamma.*rt_ij(:,3).*rt_ij(:,3) + beta);   
    ...
    % optimized layout
    Aj = gpuArray(zeros(1*N,6));                    % allocate the memory on the CPU
    ...
    Aj(1:N,3) = C.*(gamma.*rt_ij(:,1).*rt_ij(:,3)       );
    % y row
    Aj(1:N,4) = C.*(gamma.*rt_ij(:,2).*rt_ij(:,2) + beta);
    Aj(1:N,5) = C.*(gamma.*rt_ij(:,2).*rt_ij(:,3)       );
    % z row
    Aj(1:N,6) = C.*(gamma.*rt_ij(:,3).*rt_ij(:,3) + beta); 
    
    Aj(isnan(Aj))=0;                                % removing all NaN which occured due do division by zero
    
    
    %% Final Fast Fourier Transformation
    % first empty some memory
    vars = {'beta','C','gamma','r_all','r_center','r_ij','r_ij_vec','rt_ij'};
    clear(vars{:})
