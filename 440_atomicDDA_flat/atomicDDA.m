...
% [r0,r_on,lattice] = create_ParticleSpace();                                       
...
R_on_flat = repmat(r_on,3,1);                    % flatten version of it (saves transpositions)


%% Definitions of materials and the plane wave
n_s = 1.0;                                       % refractive index of the surrounding medium
n_m = n_Gold(wavelengths);                       % refractive index of the metal (Olmon values)
...
for i = 1:length(wavelengths)
    ...
    Ei_flat = exci.Einc_flat(r0);                       % flatten version of it (saves transpositions)
    ...
    %% Solve the matrix using fft on the GPU in a flatten arrangement (saves transpositions)
    tic  
    [P,relres,iter] = myqmr_GPU_flat(P, fftA, B(i), R_on_flat, Ei_flat, tol, maxit);
    fprintf('solver: %f %3u %5.1fs \n',relres ,iter, toc);
    
    % save essential polarizations
    buf = gather(P(R_on_flat));                         % buffer for the magic below (storage of the flatten results)
    P_store(:,i) = reshape(reshape(buf,[],3).',[],1); 
