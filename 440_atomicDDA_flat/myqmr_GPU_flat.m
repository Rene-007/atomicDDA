function [x,relres,iter] = myqmr_GPU_flat(P,fftA,B,R_on_flat,b_flat,tol,maxit)
%% minimal Quasi-Minimal Residual Method after Freud & Nachtigal 1991 and the Matlab QMR implementation
    ...
    x = P;
    fftA = single(gpuArray(fftA));
    B = single(gpuArray(B));
    R_on_flat = single(gpuArray(R_on_flat));
    b_flat = single(gpuArray(b_flat));
    ...
    r = gpuArray(R_on_flat.*(b_flat - (Conv3D_flat(fftA,x) + B.*x)));
    vt = r;
    wt = r;
    p = vt;
    q = wt;
    ...
    % pt = A*p + B.*p;
    % pt = R_on.(A*p + B.*p);
    pt = R_on_flat.*(Conv3D_flat(fftA,p) + B.*p);
    ...
    while iter < maxit && norm(r) > tolb
        ...
        wt = R_on_flat.*(Conv3D_flat(conj(fftA),q) + conj(B).*q);      
    ...
    relres = gather(norm(r) / norm(b_flat));
    % x = gather(x);                             % this time we leave x/P on the GPU
