function [Ex,Ey,Ez] = E_grid_cpu(r0,P,Exci,r_grid,N,M)
...
%  N/M   ... dimension of the pixel grid
    ...
    x = single(reshape(r_grid(:,1)/Exci.wav,N,M));
    y = single(reshape(r_grid(:,2)/Exci.wav,N,M));
    z = single(reshape(r_grid(:,3)/Exci.wav,N,M));

    Ex = single(zeros(N*M,1));  
    Ey = single(zeros(N*M,1));  
    Ez = single(zeros(N*M,1));  
