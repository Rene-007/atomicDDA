...
<div align="center"><img style="background: white;" src="..\003_media\QP5eCAkDjK.svg"></div>
<br/>
...
We start at the center dipole and mark the interactions with all the other dipoles (purple and red arrows). Then we move one dipole to the right and add the so far unmarked five new interactions with green arrows.

<div align="center"><img src="/003_media/conv01-03.jpg" alt="Sketch of dipole interactions."></div>

Something to highlight is, that the five red arrows to the right are not pointing onto dipoles anymore. But instead of removing them, we create dipoles with zero polarizability instead, i.e. we still consider the interactions mathematically but physically switch them off.

Let us not stop at this point and add further green arrows to the top, right as well as bottom to make the pattern symmetric and then move around to gather all possible interactions as animated below.

<div align="center"><img src="/003_media/conv_animation.gif" width="58%" alt="Animation of the convolution."></div>

This means, by expanding the original grid slightly, we are able represent all interactions with on common schematics. Note, the expanded rectangular grid has to be at least *2x* maximum width times *2x* maximum height of the structure and it has to be periodically repeated, i.e. arrows which exceed the boundary of the rectangular grid to the right can be seen as simply wrapped around and re-enter from the left.

We can now count all grid points starting with *-40* at the bottom left, *0* in the center and finishing with *40* at the top right. Using that notation we build a vector containing all positions, e.g. <!-- $\color{teal}{p_{0}}$ --> <img style="transform: translateY(0.1em); background: white;" src="..\003_media\hQhUfEroSG.svg"> is the polarization of the dipole in the center, and a vector containing all interactions, e.g. <!-- $\color{purple}{a_{10}}$ --> <img style="transform: translateY(0.1em); background: white;" src="..\003_media\6Kxxy3IMUC.svg"> is the interaction with the next dipole placed  to the top right of it. A spatial representation of these vectors looks like the following:

<div align="center"><img src="/003_media/conv_numbering.jpg" alt="New numbering of the vector and interactions."></div>

Let us now write down the original matrix equation with this new notation. Note, to avoid confusions with the original notation, lower-case letters have been used intentionally:
...
The matrix has significantly grown in size (from 21 to 42 rows/columns) but it has a very specific symmetry: all rows have the same elements and they are just circled around. Hence, it is called *circulant matrix* and one only needs to know the first row to reconstruct the whole matrix. This means, instead of *21 x 21 = 441* elements, in principle just *40 - (-40) + 1 = 81* would need to be stored for our example.

*Circulant* matrices have another very powerful property which will improve the computation time by orders of magnitude. However, this would add too much complexity for the moment. Let us rather try to stay sane (remember, this example was only a 2D simplification with scalar elements of the whole beauty) and finish off this chapter by implementing the theory we have so far.



## Code Changes

As the matrix is not symmetric anymore, the BCG solver cannot be used anymore and was removed.
...
The code in *advancedDDA.m* has some subtle changes: `r0` is now a list of grid points instead of dipoles positions and `r_on` is a list of the same length whose elements are true if there is a dipole at the specific grid position. `R_on` is the basically the same but tripled to reflect the three dimensions of  space. 

The *create_Spheroid* routine was extended to generate the new grid space as well as `r_on`. In contrast, the *create_A* function stayed the same as it automatically creates a circulant matrix with the new grid. Note, for the sake of simplicity we store the whole matrix for now which will increase the number of elements by 2 in each dimension.
...
* Secondly, as we now had to asign and hand over the diagonal matrix *B* separately, we represented it as just a vector to save some space.
...
So, after doing all these changes the overall matrix size -- as expected -- increased *8* times and, hence, the code now runs much slower. However, with the improvements in [the next section](../310_advancedDDA_FFT) the tide will turn.