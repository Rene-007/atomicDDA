...
We take again our Gold sphere with the 50-nm diameter and this time tune the dipole spacing. When going from the original 5&thinsp;nm spacing to 4&thinsp;nm and later 2.5&thinsp;nm the number of dipoles first roughly doubles from 515 dipoles to 1018 and than quadruples to 4169. This improves the quality of the spectra a lot as the unphysical ripples between 600&thinsp;nm and 750&thinsp;nm disappear -- see the spectra below. (Note, the spiky shape of the main peak disappears when using finer spectral discretizations.)

<div align="center"><img src="/003_media/sphere-50nm_invC_Comparison.jpg" alt="Results for the 50x40x30 rectangular"></div>

However, the time for solving the problem increases by ~6x and ~250x, respectively. The reason for this very bad scaling is that inverting a matrix with _NxN_ elements scales with _O(N<sup>3</sup>)_. So, when further doubling the number of dipole (spacing 2.0&thinsp;nm) and solving for the first wavelength we obtain:

    > standardDDA
    Number of dipole: 8144
    wav = 400nm -- setting up: 13.9s -- solver: 0.000000   0 260.5s 

This means, calculating the whole spectrum (41 wavelengths) would take around 3 hours and a 100-nm sphere (8x the dipoles) probably one month! This is hardly acceptable -- the computer I am using is already a state of the art *AMD Ryzen 9 5950X* with 16 cores and 32 threads. So, let us go a step back. 
...
we only have to invert <!-- $\mathbf{C}$ --> <img style="transform: translateY(0.05em);" src="..\003_media\k3DdFIe8PY.svg"> once and can use the result for simulating all different kinds of excitations, e.g. different angles of incidence & polarization, focused beams, dipole excitations and so on. This sounds in general very appealing as we only have to do the work once.  However, the _O(N<sup>3</sup>)_ scaling does not only mean that inverting the matrix is very slow but also that the result might be numerically wrong. The reason is that we are dealing with floating point numbers which produce tiny round-off errors [<img src="../003_media/External.svg" height="14">](https://en.wikipedia.org/wiki/Round-off_error) during most arithmetic operation. They are usually managable but when we deal with _O(N<sup>3</sup>)_ compuational steps that take days to compute this can easily add up to very significant errors. Hence, we might need a less general but numerical more favorable approach. 


## Iterative Algorithms

One more stable approach is to use iterative algorithms.  
...
When *|r|* becomes zero then we are at a (local) minimum and have an exact solution. If not, we try to minimize the residual to come closer to the solution. For doing that, we
...
Running the above example of a Gold sphere with the 50-nm diameter, 2.5-nm spacing and 4169 dipoles using the Sarkar CCG method gives the following results:
...
One can see that the CCG solver is initially much faster than the inverting-the-matrix method but degrades rapidely for longer wavelengths. Alltogether it is still faster (1351.6&thinsp;s vs. 1633.9&thinsp;s) for this small example and the larger the problem the clearer the win (50-nm diameter, 2.0-nm spacing and 8144 dipoles):
...
Here even the slowest CCG wavelength at 800&thinsp;nm is faster than the inverting-the-matrix method constant 260&thinsp;s per step. Bigger problems would just increase this gap. Furthermore, switching to another iterative solver can also give a little boost: For the smaller example the QMR solver only needs 917.9&thinsp;s and the BCG (Sarkar) 970.8&thinsp;s.
